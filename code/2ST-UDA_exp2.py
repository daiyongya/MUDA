...
from data_prep.folded_dataset import FoldedDataset
...
def train(train_sets, dev_sets, test_sets, threshold, log):

    # ---------------------- dataloader ---------------------- #
    train_loaders = DataLoader(train_sets, opt.batch_size, shuffle=False, drop_last=True)
    dev_loaders = DataLoader(dev_sets, opt.batch_size, shuffle=False, drop_last=True)
    ...
    # 这边要改动比较大，直接把folded_dataset给去掉，用tensor dataset就行
    is_first = True
    co_training_targets, co_training_pseudo_labels = None, None
    init_samples = len(train_sets)
    # 实验二这里get fake label的是target domain的2000个样本
    # 这边要想办法改一下
    gen_label_dataset = copy.deepcopy(train_sets)
    print("**************************************")
    ...
    while threshold > 0.5 and odd_step_labels + even_step_labels > 10:
        ...
        # ---------------------- dataloader ---------------------- #
        train_loaders_copy = DataLoader(gen_label_dataset, opt.batch_size, shuffle=False, drop_last=True)
        ...
        for i in integrated_sample_index_list[final_sample_index_list_index].numpy().tolist():
            x.append(gen_label_dataset[i][0])

        print(len(x))
        print('*' * 60)
        # torch.tensor要转换的list里面的元素包含多维的tensor
        # 下面的语句会报错：ValueError: only one element tensors can be converted to Python scalars
        # x = torch.tensor([item for item in x]).float().to(opt.device)
        x = torch.stack(x).float().to(opt.device)
        if final_pseudo_labels.numel() != 1:
            y = final_pseudo_labels.long().to(opt.device)
        temp_train_sets = TensorDataset(x, y)
        ...
        target_dataloader_labelled = DataLoader(temp_train_sets, opt.batch_size, shuffle=False)
        ...
        F_d_target.load_state_dict(
            torch.load(os.path.join(opt.exp2_model_save_file, f'net_F_d_target.pth')))
        ...
        for i in remain_samples_index.tolist():
            x.append(gen_label_dataset[i][0])
            y.append(gen_label_dataset[i][1])

        x = torch.stack(x).float().to(opt.device)
        y = torch.stack(y).long().to(opt.device)
        # print(x.shape)
        # print(y.shape)
        gen_label_dataset = TensorDataset(x, y)
    ...
    opt.max_epoch = 50
    total_train_sets_x = torch.stack(total_train_sets_x).float().to(opt.device)
    total_train_sets_y = total_train_sets_y.long().to(opt.device)
    total_train_sets = TensorDataset(total_train_sets_x, total_train_sets_y)
    target_dataloader_labelled = DataLoader(total_train_sets, opt.batch_size, shuffle=True)
    ...
    F_d_target.load_state_dict(torch.load(os.path.join(opt.exp2_model_save_file, f'net_F_d_target.pth')))
    ...
    for epoch in range(opt.max_epoch):
        ...
        if dev_acc > best_dev_acc:
            ...
            torch.save(F_d_target.state_dict(),
                       '{}/net_F_d_target.pth'.format(opt.exp2_model_save_file))
...
def main():
    ...
    for target_domain in all_target_domains:
        ...
        logging.basicConfig(stream=sys.stderr, level=logging.DEBUG if opt.debug else logging.INFO)
        log = logging.getLogger(__name__)
        fh = logging.FileHandler(os.path.join(opt.exp2_model_save_file, '2ST-UDA_exp2_2020_11_15.txt'))
        log.addHandler(fh)
        # output options
        log.info(opt)
        ...
        test_sets, datasets = get_msda_amazon_datasets(
        ...
        # train_samples = 0.8 * len(datasets)
        # train_sets, dev_sets = datasets[:train_samples], datasets[train_samples:]
        # train_sets, dev_sets = datasets, datasets
        train_sets, dev_sets = datasets, datasets
        # ---------------------- 训练产生伪label和F_d_target的过程 ---------------------- #
        # 在两阶段训练这里，两种方式处理：
        # 1. 把target domain的2000张拆分成训练集和验证集，4000+张作为测试集      (写一下这个)
        # 2. 不设置验证集，只留下target domain的2000张作为训练集，然后4000+张作为测试集  (这个不太好写)
        test_acc = train(train_sets, dev_sets, test_sets, threshold, log)
