...
# 这边代码是只有前向推断的步骤
def train(vocab, train_sets, dev_sets, test_sets, log):
...
def main():
    ...
    for target_domain in all_target_domains:
        ...
        logging.basicConfig(stream=sys.stderr, level=logging.DEBUG if opt.debug else logging.INFO)
        log = logging.getLogger(__name__)
        fh = logging.FileHandler(os.path.join(opt.exp3_model_save_file, 'WS-UDA_log.txt'))
        log.addHandler(fh)
        # output options
        log.info(opt)
        ...
        training_data_label_correct_acc, test_data_label_correct_acc = train(vocab, train_sets, dev_sets, test_sets, log)
