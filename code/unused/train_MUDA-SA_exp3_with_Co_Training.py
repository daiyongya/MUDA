...
def train(vocab, train_sets, dev_sets, test_sets, threshold, log):
    ...
    while threshold > 0.5 and odd_step_labels + even_step_labels > 10:
        ...
        if final_pseudo_labels is None or final_pseudo_labels.numel() == 1:               # 一个样本都没有的情况或者只有一个样本的情况
            threshold -= 0.02
            continue
...
def main():
    ...
    for target_domain in all_target_domains:
        ...
        base_save_dir = "/hdd/liujian/AAAI-2020-Supplementary-Experiment/"
        opt.exp3_model_save_file = base_save_dir + "2019_11_18/source_target_only_unlabeled_data_domain_hidden_size_128_without_double_data"

        # ---------------------- 设置log ---------------------- #
        logging.basicConfig(stream=sys.stderr, level=logging.DEBUG if opt.debug else logging.INFO)
        log = logging.getLogger(__name__)
        fh = logging.FileHandler(os.path.join(opt.exp3_model_save_file, 'Cotraining_log_2019_11_21.txt'))
        log.addHandler(fh)
        # output options
        log.info(opt)
