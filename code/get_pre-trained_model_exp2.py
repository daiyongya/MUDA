...
def train(train_sets, dev_sets, test_sets, unlabeled_sets, log):
    """
    ...
    for domain in opt.target_domains:
    ...
    for epoch in range(opt.max_epoch):
        ...
        avg_acc = sum([acc[d] for d in opt.target_domains]) / len(opt.target_domains)
        ...
        for domain in opt.target_domains:
            test_acc[domain] = evaluate(domain, test_loaders[domain],
                                        F_s, F_d[domain] if domain in F_d else None, C, log)
        avg_test_acc = sum([test_acc[d] for d in opt.target_domains]) / len(opt.target_domains)
...
def main():
    ...
    for target_domain in all_target_domains:
        ...
        fh = logging.FileHandler(os.path.join(opt.exp2_model_save_file, 'log.txt'))
        log.addHandler(fh)
        # output options
        log.info(opt)
        ...
        for domain in opt.all_domains:
            datasets[domain], raw_unlabeled_sets[domain] = get_msda_amazon_datasets(
                opt.prep_amazon_file, domain, opt.feature_num)
        log.info(f'Done Loading {opt.dataset} Datasets.')
        log.info(f'Domains: {opt.all_domains}')

        # 这里要稍微改一改，分清楚train dev test
        # train为三个source domain的训练数据集，dev为target domain的训练数据集，test为target domain的raw_数据集
        ...
        cv = train(train_sets, dev_sets, test_sets, unlabeled_sets, log)
